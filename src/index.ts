...
interface Pipeline<R=any, W=any> {
    readable:ReadableStream<R>;
    writable:WritableStream<W>;
    pipe<T>(next:Pipeline<T, R>):Pipeline<T, W>;
...
export function Stream<R> (
    ...
    return {
        ...
        pipe<T> (next:Pipeline<T, R>):Pipeline<T, never> {
            ...
            return {
                ...
                writable: null as any,
                pipe<U> (another: Pipeline<U, T>):Pipeline<U, never> {
...
export function through<I, O> (
    ...
    return {
        ...
        pipe<T> (next:Pipeline<T, O>):Pipeline<T, I> {
            ...
            return {
                readable: pipedReadable,
                writable: transformStream.writable,
                pipe<U> (another: Pipeline<U, T>):Pipeline<U, I> {
 ...
 */
export async function collect<T> (stream:Pipeline<T, any>):Promise<T[]> {
    const results:T[] = []
 ...
 */
export async function run<T> (stream:Pipeline<T, any>):Promise<void> {
    const reader = stream.readable.getReader()
    try {
        while (true) {
